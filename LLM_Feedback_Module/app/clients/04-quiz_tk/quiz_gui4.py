...
class QuizGUI:
    ...
    def create_widgets(self):
        ...
        self.question_type_label = ttk.Label(self.header_frame, text="Question Type:", font=("Arial", 12, "bold"))
        self.question_type_label.pack(side=tk.LEFT)

        self.question_type_combobox = ttk.Combobox(
            self.header_frame, textvariable=self.question_type, state="readonly"
        )
        self.question_type_combobox.pack(side=tk.LEFT)
        self.question_type_combobox.bind("<<ComboboxSelected>>", self.on_question_type_selected)
    ...
    def show_existing_quizzes(self):
        ...
        self.question_type_combobox['values'] = ['All', 'Open-Ended', 'Multiple Choice']
        self.question_type_combobox.set('All')  # Default to showing all questions

    def show_quiz_buttons(self):
        self.submit_button.pack(side=tk.LEFT)
        self.next_button.pack(side=tk.RIGHT)

    def on_question_type_selected(self, event):
        selected_type = self.question_type.get()
        if selected_type == 'Open-Ended':
            filter_type = 'open_ended'
        elif selected_type == 'Multiple Choice':
            filter_type = 'multiple_choice'
        else:
            filter_type = 'all'  # For 'All' option

        # Check if a quiz is currently active
        if not self.quiz_app.current_quiz:
            messagebox.showinfo("Filter", "Select the Quiz First To Filter Questions")
            return

        # Update the display logic based on the selected question type
        self.quiz_app.filter_questions_by_type(filter_type)
        self.show_next_question()

    def _map_type_to_filter(self, type_str: str) -> str:
        """Map the combobox selection to filter type."""
        if type_str == 'Open-Ended':
            return 'open_ended'
        elif type_str == 'Multiple Choice':
            ...
            if (
                ...
                if answer:
                    self.quiz_app.submit_answer(
                        self.current_question.question_id, open_ended_response=answer
                    )
                    self.show_next_question()
                else:
                ...
                if selected:
                    self.quiz_app.submit_answer(
                        self.current_question.question_id, selected_choice=selected
                    )
                    self.show_next_question()
                else:
                    messagebox.showwarning(
