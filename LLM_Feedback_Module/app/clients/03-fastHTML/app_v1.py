...
app.state.submitted_answers = {}
app.state.current_answers = {}
answers_db = {}
...
def pagination_button_component(prev_text, next_text, prev_page, next_page, show_submit):
    ...
    submit_button = Button( "Submit",
                            hx_post="/submit",
                            hx_target="#quiz-content",
                            cls="join-item btn btn-primary") if show_submit else None
...
def quiz_body_component(question_number, total_questions, question, options, option_type):
    ...
    current_answer = app.state.current_answers.get(question_number - 1, '')

    _radio_button_options = Div(
        *[
            Div(
                Label(
    ...
    if option_type == "select_one":
        options_component = _radio_button_options
    elif option_type == "select_multiple":
        options_component = _checkbox_options
    elif option_type == "enter_text":
        options_component = _text_input_option
    else:
    ...
    return Form(
        Div(
            ...
            Div(f"Q. {question}",
                cls=
                "border bg-[#f3f4f5] shadow-lg my-7 rounded px-7 py-5 font-semibold text-xl"
                ),
            ...
            options_component,
            cls="border rounded-md min-h-96 shadow-md py-6 px-4 ", id="quiz-container"))
...
updateCountdown();
""" % time_left_global.total_seconds()
...
def store_answer(app, question_number, answer):
    app.state.submitted_answers[question_number] = answer
...
async def quiz_page_with_number(request: Request, page_number: int):
    ...
    for key, value in form_data.items():
        if key.startswith('q'):
            ...
            app.state.current_answers[question_number] = value
    ...
    for key, value in form_data.items():
        if key.startswith('q'):
            ...
            if isinstance(value, list):
                app.state.submitted_answers[question_number] = value
            else:
                app.state.submitted_answers[question_number] = value

    # Copy current_answers to submitted_answers
    for key, value in app.state.current_answers.items():
        if key not in app.state.submitted_answers:
            app.state.submitted_answers[key] = value

    return RedirectResponse('/results', status_code=303)

def results_summary_component(answers):
    return Div(
        H2("Quiz Results Summary", cls="text-2xl font-bold mb-4"),
        *[Div(
            P(f"Question {i+1}: {questions[i]['question']}", cls="font-semibold"),
            P(f"Your answer: {answers.get(i, 'Not answered')}",
...
async def get_results(request: Request):
    return Div(
        results_summary_component(request.app.state.submitted_answers),
...
if __name__ == '__main__':
    uvicorn.run("app_v1:app", host="127.0.0.1", port=8001, reload=True)